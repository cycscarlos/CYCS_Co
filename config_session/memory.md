...
- 524f5e1 — feat: vista contacto — botón Enviar en `--brand-primary`, socials centrados con gradiente de Instagram visible desde el inicio y hover con halo; se eliminan linkedin y whatsapp del bloque de redes. **← HEAD actual**
...
## Pendiente
- **Push pendiente**: rama `main` adelantada respecto a `origin/main` (**19 commits**, hasta 524f5e1; los huérfanos NO se pushean). El push lo ejecuta el usuario (decidido: se pushea).
- **Probar en `npm run dev`** (el usuario): navbar/componentes en las 4 vistas técnicas + contacto bajo `/CYCS_Co/`, y el nuevo bloque de socials de contacto (solo Instagram, gradiente y hover).
- **Investigación del usuario**: Chaport (panel, plan, sustitución/centralización del snippet). Decidir si se trabaja algo tras su informe.
- **Leftovers** (a decidir): `assets/css/components/footer.css` con una línea comentada `/* background-color: rgba(8,14,24,.8); */` (resto de experimento); borrado de `CYCS_Co-v2.code-workspace`; archivos untracked `AGENTS.md`, `config_session/`, `opencode.json`, `x-CYCS_Co-v2.code-workspace` (locales, no se commitean).
